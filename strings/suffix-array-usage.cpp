...
vector<int> suffix_array(string s) {
  ...
  for(int i=0; i<20; i++) {
    ...
    //.second
    for(int j=0; j<1024; j++) ps[j] = 0;
    for(int j=0; j<n; j++) {
      int tar = v[j].first + (1 << i);
      tar %= n;
      v[j].second.second = v[pos[tar]].second.first;
      ps[v[j].second.second & 1023]++;
    }
    for(int j=1; j<1024; j++) ps[j] += ps[j-1];
    for(int j=n-1; j>=0; j--) {
      ps[v[j].second.second & 1023]--;
      w[ps[v[j].second.second & 1023]] = v[j];
    }
    for(int j=0; j<1024; j++) ps[j] = 0;
    for(int j=0; j<n; j++) ps[w[j].second.second >> 10]++;
    for(int j=1; j<1024; j++) ps[j] += ps[j-1];
    for(int j=n-1; j>=0; j--) {
      ps[w[j].second.second >> 10]--;
      v[ps[w[j].second.second >> 10]] = w[j];
    }
    //.first
    for(int j=0; j<1024; j++) ps[j] = 0;
    for(int j=0; j<n; j++) ps[v[j].second.first & 1023]++;
    for(int j=1; j<1024; j++) ps[j] += ps[j-1];
    for(int j=n-1; j>=0; j--) {
      ps[v[j].second.first & 1023]--;
      w[ps[v[j].second.first & 1023]] = v[j];
    }
    for(int j=0; j<1024; j++) ps[j] = 0;
    for(int j=0; j<n; j++) ps[w[j].second.first >> 10]++;
    for(int j=1; j<1024; j++) ps[j] += ps[j-1];
    for(int j=n-1; j>=0; j--) {
      ps[w[j].second.first >> 10]--;
      v[ps[w[j].second.first >> 10]] = w[j];
    }
    newClasses[0] = 0;
    for(int j=1; j<n; j++) {
