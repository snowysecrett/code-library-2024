struct flow_graph_construct {
  struct edge {
    int to, cost, flow, revid; // reverse edge id in adjacency list
  };
  vector<unordered_map<int, int> > solution;
  ...
  int query_edge(int u, int v) { // Total flow that has passed through edge (u, v)
    return solution[u][v] - solution[v][u];
  }
};
